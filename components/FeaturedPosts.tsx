...
const renderBigItem: FC<Post> = (post) => {
    return (
        <div key={post.id} className='relative col-span-2 row-span-2 overflow-hidden rounded-md md:col-span-2 md:row-span-2'>
...
const renderItem: FC<Post> = (post) => {
    return (
        <div key={post.id} className='relative overflow-hidden rounded-md'>
            <Image src={post.coverImage.url} layout='fill'/>
